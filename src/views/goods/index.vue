<template>
  <div>
    ...
    <!-- 数据列表区域 -->
    <listComp ref="Mylist" :list="list" @getList="getGoodsList"></listComp>
    <!-- 表单区域：添加|编辑 -->
    <formComp
      ref="Myform"
      :isShow.sync="isShow"
...
export default {
  ...
  methods: {
    ...
    getSpecsList() {
      this.$serve.getSpecsList().then((res) => {
        this.specsList = res.list;
      });
    },
    getGoodsList() {
      this.$serve.getGoodsList(this.params).then((res) => {
        this.list = res.list;
        this.getParamsTotal();
      });
    },
    getParamsTotal() {
      this.$serve.getGoodsCount().then((res) => {
